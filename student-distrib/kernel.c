...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        start_addr = (boot_block_t*) mod->mod_start;
    ...
    /* Init the keyboard */
    keyboard_init(); //enable / disable for keyboard
    ...
    rtc_init(); //enable / disable for rtc

    /* Init the file_system */
    file_sys_init(start_addr);
    
    /* init fops table */
    init_fops_table();

    /* init terminals with pcb vals of null */
    terminal_init();
